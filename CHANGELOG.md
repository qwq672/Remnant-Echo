...
### 新增（v0.9 设计问答第三批 · 2026-09-21 第三批）

**新增设计问答第三批文档**：
- `planning/24-design-followups-2.md` — 项目方 9-21 对模组命名最终确认+定位扩展+6 个新问题
  - **模组命名最终确认**：`Remnant Echo`（组合名，避开重名）
  - **模组定位扩展**：不止是叙事——是"补齐整个原版世界观"+"像电影一样"+"剧情不无聊"
  - Q13 主线 Boss：凋灵+监守者+末影龙 三主线必经 Boss（按 07 文档已绑定）
  - Q14 剧情顺序简聊：按 02/03/21 已有规划
  ...
  - Q18 粒子特效：L1/L2/L3 三级粒子策略（~70%/~20%/~10%），5 个 L3 自定义粒子候选（withering_wave/sculk_echo/pale_regret/colonial_echo/m_farewell）

**更新根目录文档**：
- `BLUEPRINT.md` — 标题改为"Remnant Echo · 项目蓝图总览"，类型描述扩展为"补齐整个原版世界观"+"像电影一样"+"剧情不无聊"，命名计划改为已确认 Remnant Echo
- `README.md` — 标题改为"Remnant Echo · Minecraft 原版世界观补齐资料汇编"，模组项目信息更新

### 关键设计决策（v0.9 设计问答第三批 · 2026-09-21）
- 模组命名最终确认为 **Remnant Echo**——避开重名，含义为"先民后裔聆听先民余响"
- 模组定位扩展为"补齐整个原版世界观"+"像电影一样的电影化碎片叙事"+"剧情不无聊"
- 主线 Boss 三件套：凋灵（玩家主动召唤）+监守者（玩家意外遭遇）+末影龙（玩家主动挑战）
- 严格禁止预渲染视频与录制回放（违反原则 0/3）
- 接受电影化碎片叙事（L1+L2 particle+/playsound+/time set+多重 tellraw）
- L3 自定义"叙事讲台"作为可选彩蛋（v0.6 真结局时评估）
- 光影提示通过 advancement+ tellraw 显示一次性，玩家可关闭
- 粒子特效采用 L1/L2/L3 三级策略，5 个 L3 自定义粒子候选（基于原版粒子衍生）

### 新增（v1.0 设计问答第四批 · 2026-09-21 第四批）

**新增设计问答第四批文档**：
- `planning/25-design-followups-3.md` — 项目方 9-21 对命令工程化+生物变体+动画的 3 个关键问题
  - **Q19 命令工程化**：项目方担心纯命令"太那啥"——采用 E1/E2/E3 三级工程化策略
    - E1（~30%）：纯 datapack（function + advancement + tellraw）
    - E2（~50%）：Java 封装 function 调用，玩家完全感知不到命令
    - E3（~20%）：Java Mixin 拦截原版事件，精确时机控制
  - **Q20 叙事生物变体**：原版村民手分不开，无法做"递笔记"等关键叙事动作——4 个 L3 自定义叙事 NPC 候选
    - 古老图书管理员（v0.1 必备）
    - 考古学家村民（v0.1 必备）
    - 流浪者长老（v0.2）
    - 末地殖民军后裔（v0.5 真结局核心）
    - 所有变体基于原版生物纹理衍生，行为与原版一致，退化时退化为原版生物
  - **Q21 动画策略**：L1/L2/L3 三级动画策略
    - L1+L2（~80%）：原版动画 + particle 模拟动作（particle_note/happy_villager 等）
    - L3 Mixin（~20%）：仅用于 4 个叙事 NPC 的关键动作（递笔记/指路），不修改原版 AI
    - 替代方案：L3 自定义"叙事序列 function"（用原版 particle+effect+playsound+tellraw 实现"电影化序列"，避免 Mixin 复杂性）

### 关键设计决策（v1.0 设计问答第四批 · 2026-09-21）
- 模组采用 E1/E2/E3 三级工程化架构（datapack+Java 封装+Mixin 拦截混合），玩家感知不到命令存在
- 启动 4 个 L3 自定义叙事 NPC（基于原版生物纹理衍生，不替换原版村民）
- 动画策略：L1+L2 优先（80%场景），L3 Mixin 仅用于关键叙事动作（20%场景）
- 模组定位从"纯叙事模组"进一步扩展为"叙事+工程化 Fabric mod"，玩家体验与专业模组一致

### 新增（v1.1 国内社区传说汇编 · 2026-09-21）

**新增国内社区传说文档**：
- `15-community-lore/05-chinese-community-theories.md` — 基于项目方 9-21"看看国内社区"指示，6 次真实 web 搜索汇编国内 Minecraft 社区传说
  - **MCBBS 关闭说明**：2024 年国内最大 Minecraft 论坛关闭，部分历史资料被网友备份
  - **MC百科（mcmod.cn）**：当前国内最大 Minecraft 模组百科，设定引用权威
  - **萌娘百科**：监守者早期译名"循声守卫"的历史记录
  - **中文 Minecraft Wiki（zh.minecraft.wiki）**：原版设定中文权威
  - **知乎**：末影人怕水理论（Burnie 儿子提议的开发故事，模组不采纳）
  - **百度贴吧 minecraft 吧**：仍活跃，但讨论质量参差
  - **B 站**：舞秋风等 UP 主的远古城市解析视频
  - **Reddit 中文翻译版**：2019 年古代建造者理论中文讨论 + 2024 年"远古城市=跨维度收集工具"理论
  - **国内剧情整合包参考**：VNDialog/落雪长冬/Story Mode Remastered——差异化定位为"碎片化叙事而非对话框"
  - 国内社区理论共 9 个（3 完全采纳+3 部分采纳+3 不采纳）

**更新章节索引**：
- `15-community-lore/00-index.md` — 目录新增 05，资料权威性分级新增国内社区条目（★★★），真实搜索记录从 8 次增至 14 次

### 关键设计决策（v1.1 国内社区传说 · 2026-09-21）
- 国内社区与国际社区形成跨语言共识——三界同源、古代建造者、远古城市=跨维度收集工具 等理论在中英文社区均有讨论
- 模组不采纳知乎"末影人怕水=开发故事"作为叙事依据——仍按"维度异化"解读
- 国内剧情整合包市场证明国内玩家对叙事模组有需求，但模组差异化定位为"碎片化叙事"而非"对话框叙事"
- 项目方明确"先不考虑写 Java 代码"——继续以收集资料+定剧情为主
