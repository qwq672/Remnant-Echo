...
#### Level 1（优先）：原版 advancement + tellraw
...
#### Level 2（增强）：原版末地传送门 frame 行为变体（备选）

若 Level 1 的提示效果不足，可考虑：
- 监听玩家进入末地传送门的事件
- 若未达标，触发原版 `particle` 命令显示"黑雾"粒子效果（使用原版 `minecraft:dragon_breath` 或 `minecraft:squid_ink` 粒子）
- 玩家仍可进入末地，但视觉上有"反噬"反馈

**评估**：
- ✅ 用原版 `particle` 命令，不新增自定义粒子
- ✅ 不阻止玩家进入，仅视觉反馈
- ⚠️ 需监听玩家传送事件，属于"机制扩展"但非"新机制"

#### Level 3（必要时）：自定义末地传送门 frame 变体（暂不实施）

仅在 Level 1+2 效果不足时考虑：
- 自定义一个"残破末地传送门 frame"方块变体
- 材质基于原版 `end_portal_frame` 衍生（添加裂纹纹理）
- 未达标玩家右键时显示"黑雾反噬"粒子 + 提示文本
- 达标玩家右键时正常激活

**评估**：
- ✅ 材质基于原版 `end_portal_frame` 衍生，符合"原版关联性"
- ⚠️ 自定义方块，属于 Level 3
- ⚠️ 改变原版末地传送门交互，可能违反"不魔改原版交互"
- **结论**：暂不实施，仅在 v0.5 末地线开发时重新评估

### 6.3 兼容性分析

- ✅ 不强制阻止玩家（只是提示）
- ✅ 不伤害玩家
- ✅ Level 1 不改变原版末地传送门机制
- ✅ Level 1 不新增自定义机制（仅用原版 advancement + tellraw）
- ✅ 玩家可通过 `/gamemode` 等指令绕过，符合"不强制阻止"原则
- ✅ 完全符合"补优先于添"——Level 1 优先用原版机制，Level 2/3 作为备选
...
| 2026-09-18 | v0.3 | 修订原则为「补优先于添」三级优先级，认知锁机制分级实现（Level 1 优先用 advancement，Level 2/3 作为增强/备选） | 项目方 |
